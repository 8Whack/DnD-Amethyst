axios.get('https://www.dnd5eapi.co/api/classes')
    .then((res)=>{
        ...
        for(i=0; i< res.data.count; i++){
            newClass = document.createElement('li');
            newClass.textContent = res.data.results[i].name;
            document.getElementById('classList').appendChild(newClass)
        }
    })
    .catch((err)=>{
...
document.getElementById('classSearchBtn').addEventListener('click', classSearch)